...
// Helper to create an AbortSignal for renderOutputItem calls
...
suite('Notebook builtin output renderer', () => {
	...
	textLikeMimeTypes.forEach((mimeType) => {
		test(`Render with wordwrap and scrolling for mimetype ${mimeType}`, async () => {
			...
			await renderer!.renderOutputItem(outputItem, outputElement, createAbortSignal());
		...
		test(`Replace content in element for mimetype ${mimeType}`, async () => {
			...
			await renderer!.renderOutputItem(outputItem2, outputElement, createAbortSignal());
	...
	test(`Appending multiple streaming outputs`, async () => {
		...
		await renderer!.renderOutputItem(outputItem1, firstOutputElement, createAbortSignal());
		const secondOutputElement = outputHtml.appendOutputElement();
		await renderer!.renderOutputItem(outputItem2, secondOutputElement, createAbortSignal());
		const thirdOutputElement = outputHtml.appendOutputElement();
		await renderer!.renderOutputItem(outputItem3, thirdOutputElement, createAbortSignal());

		const appendedItem1 = createOutputItem('', stdoutMimeType, '1', ' appended1');
		await renderer!.renderOutputItem(appendedItem1, firstOutputElement, createAbortSignal());
		const appendedItem3 = createOutputItem('', stdoutMimeType, '3', ' appended3');
		await renderer!.renderOutputItem(appendedItem3, thirdOutputElement, createAbortSignal());
	...
	test(`Multiple adjacent streaming outputs should be consolidated one element`, async () => {
		...
		await renderer!.renderOutputItem(outputItem1, outputElement, createAbortSignal());
		await renderer!.renderOutputItem(outputItem2, outputHtml.appendOutputElement(), createAbortSignal());
		await renderer!.renderOutputItem(outputItem3, outputHtml.appendOutputElement(), createAbortSignal());
	...
	test(`Consolidated streaming outputs should replace matching outputs correctly`, async () => {
		...
		await renderer!.renderOutputItem(outputItem2, secondOutput, createAbortSignal());
		const newOutputItem1 = createOutputItem('replaced content', stdoutMimeType, '2');
		await renderer!.renderOutputItem(newOutputItem1, secondOutput, createAbortSignal());
	...
	test(`Consolidated streaming outputs should append matching outputs correctly`, async () => {
		...
		await renderer!.renderOutputItem(appendingOutput, secondOutput, createAbortSignal());
	...
	test(`Multiple adjacent streaming outputs, rerendering the first should erase the rest`, async () => {
		...
		await renderer!.renderOutputItem(newOutputItem1, outputElement, createAbortSignal());
	...
	test(`Settings event change listeners should not grow if output is re-rendered`, async () => {
		...
		const outputElement = new OutputHtml().getFirstOuputElement();
		await renderer!.renderOutputItem(createOutputItem('content', stdoutMimeType), outputElement, createAbortSignal());
